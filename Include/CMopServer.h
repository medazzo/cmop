...
class IpAddress {
 public:
  ...
  IpAddress(unsigned char a, unsigned char b, unsigned char c,
            unsigned char d) {
    m_Address = (((unsigned long) a) << 24) | (((unsigned long) b) << 16)
...
class IHTTPHandler {
 public:
  ...
  Result ServeFile(const ::NPT_HttpRequest &request,
                   const ::NPT_HttpRequestContext &context,
                   ::NPT_HttpResponse &response, const char *file_path,
...
class CSFactory {
 public:
   ...
   */
  ICServer* getCServer(IpAddress listen_address, unsigned short listen_port,
