...
static void sig_term(int sig) {
  Test_UNUSED(sig);
  STATE = 0;
  NPT_LOG_INFO_1("we have received a SIG TERM :%d!!",STATE);NPT_LOG_INFO("put a choice:\n 1 :To Send Event.\n 2: To re-arm for events.\n 3:: To exit.\n " );
  scanf("%d", &STATE);
}
/*----------------------------------------------------------------------
 |   main
...
void SetAsterisk(cmop::ICServer *server) {
   ...
   *                ---- *
   */
  server->AddHandler(astrix), server->AddHandler(error);
  server->AddHandler(security);
  server->AddHandler(root);
}
void SetTT(cmop::ICServer *server) {
  OnAllHandler *root = new OnAllHandler("/api/voila");
...
int main(int argc, char **argv) {
  ...
  cmop::ICServer *server = cmop::CSFactory::Instance().getCServer(
      cmop::CMOP_INTERFACE_ANY, 1234, 3);
  cmop::ICServer *server2 = cmop::CSFactory::Instance().getCServer(
      cmop::CMOP_INTERFACE_ANY, 7777, 3);
