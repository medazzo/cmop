...
/*----------------------------------------------------------------------
 ...
 +---------------------------------------------------------------------*/
HTTPServer::HTTPServer(NPT_IpAddress listen_address, NPT_UInt16 listen_port,
...
/*----------------------------------------------------------------------
 ...
 +---------------------------------------------------------------------*/
bool HTTPServer::FindChildNode(NPT_List<NPT_String> Segments,
                               HTTPNode *&found) {
...
void HTTPServer::ProcessClientData(HTTPServerTask *task) {
  ...
  if (taskData != NULL) {
    ...
    if (task != NULL) {
      ...
      HTTPServerTask *tt = new HTTPServerTask(taskData->m_input,
                                              taskData->m_output,
                                              taskData->m_context, this);
...
Result HTTPServer::GetNewClient() {
  ...
  {
    NPT_AutoLock lock(m_TasksLock);
    result = this->m_DataTasksWaiting->Add(
        new HTTPServerTaskData(input, output, &context));
  ...
  if ((idle == true)
      || (this->m_DataTasksWorkers->GetItemCount() < m_MaxTasks)) {
