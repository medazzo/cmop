...
void HTTPServerTask::Run() {
  ...
  } while (m_running); NPT_LOG_WARNING_1( " Task [%llu] is Stopped !! ",this->GetCurrentThreadId());
}
...
NPT_Result HTTPServerTask::RespondToClient() {
  ...
  response->GetHeaders().SetHeader(NPT_HTTP_HEADER_SERVER,
                                   HTTPUtility::HTTP_SERVER_NAME_VERSION_NUMBER,
                                   false);
  ...
  if (!NPT_FAILED(result)) {
    ...
    resultsearch = this->m_server->FindChildNode(path, found);
  ...
  if (found) {
    ...
    if (found->getNodeHandler()->GetMyHandlerType()
        == HANDLER_EVENT /*TODO add check for method support*/) {
  ...
  if ((result == NPT_ERROR_NO_SUCH_ITEM) || (found == NULL)
      || (resultsearch == false)) {
